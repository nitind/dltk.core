...
/**
 * <p>
 * This operation creates a compilation unit (CU). If the CU doesn't exist yet,
 * a new compilation unit will be created with the content provided. Otherwise
 * the operation will override the contents of an existing CU with the new
 * content.
 * 
 * <p>
 * Note: It is possible to create a CU automatically when creating a class or
 * interface. Thus, the preferred method of creating a CU is to perform a create
 * type operation rather than first creating a CU and secondly creating a type
 * inside the CU.
 * 
 * <p>
 * Required Attributes:
 ...
 * <li>The name of the compilation unit. Do not include the <code>".java"</code>
 * suffix (ex. <code>"Object"</code> - the <code>".java"</code> will be added
 * for the name of the compilation unit.)
 * <li>
 * </ul>
 */
public class CreateSourceModuleOperation extends ModelOperation {
	 ...
	 */
	public CreateSourceModuleOperation(IScriptFolder parentElement,
			String name, String source, boolean force) {
				...
				if (force) {
					...
					if (!Util.isExcluded(unit) && unit.getParent().exists()) {
						for (int i = 0; i < resultElements.length; i++) {
							delta.changed(resultElements[i],
					...
					throw new ModelException(new ModelStatus(
							IModelStatusConstants.NAME_COLLISION, Messages
									.bind(Messages.status_nameCollision,
											compilationUnitFile.getFullPath()
													.toString())));
				}
			} else {
				try {
					...
					try {
						encoding = folder.getDefaultCharset(); // get folder
						// encoding as
						// file is not
						// accessible
					...
					InputStream stream = new ByteArrayInputStream(
							encoding == null ? fSource.getBytes() : fSource
									.getBytes(encoding));
