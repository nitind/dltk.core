...
public final class ObjectVector {
	...
	public void add(Object newElement) {
		...
		if (this.size == this.maxSize) // knows that size starts <= maxSize
			System.arraycopy(this.elements, 0,
					(this.elements = new Object[this.maxSize *= 2]), 0,
					this.size);
	...
	public void addAll(Object[] newElements) {
		...
		if (this.size + newElements.length >= this.maxSize) {
			maxSize = this.size + newElements.length; // assume no more elements
			// will be added
			System.arraycopy(this.elements, 0,
					(this.elements = new Object[this.maxSize]), 0, this.size);
		}
		System.arraycopy(newElements, 0, this.elements, size,
				newElements.length);
	...
	public void addAll(ObjectVector newVector) {
		...
		if (this.size + newVector.size >= this.maxSize) {
			maxSize = this.size + newVector.size; // assume no more elements
			// will be added
			System.arraycopy(this.elements, 0,
					(this.elements = new Object[this.maxSize]), 0, this.size);
		}
		System.arraycopy(newVector.elements, 0, this.elements, size,
				newVector.size);
	...
	public Object remove(Object element) {
		...
		for (int i = this.size; --i >= 0;)
			if (element.equals(this.elements[i])) {
				...
				System.arraycopy(this.elements, i + 1, this.elements, i,
						--this.size - i);
