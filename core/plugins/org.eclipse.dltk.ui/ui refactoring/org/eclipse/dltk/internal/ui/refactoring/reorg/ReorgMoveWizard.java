	...
	private static class MoveInputPage extends ReorgUserInputPage {
		...
		protected boolean performFinish() {
			return super.performFinish()
					|| getScriptMoveProcessor().wasCanceled(); // close the
																// dialog if
																// canceled
		}
		...
		protected RefactoringStatus verifyDestination(Object selected)
			...
			if (selected instanceof IModelElement)
				refactoringStatus = processor
						.setDestination((IModelElement) selected);
			else if (selected instanceof IResource)
				refactoringStatus = processor
						.setDestination((IResource) selected);
			else
				refactoringStatus = RefactoringStatus.createFatalErrorStatus(
						ReorgMessages.ReorgMoveWizard_4);
		...
		private void updateUIStatus() {
			...
			if (fReferenceCheckbox != null) {
				...
				processor.setUpdateReferences(fReferenceCheckbox.getEnabled()
						&& fReferenceCheckbox.getSelection());
			}
			if (fQualifiedNameCheckbox != null) {
				...
				if (enabled) {
					fQualifiedNameComponent
							.setEnabled(processor.getUpdateQualifiedNames());
				...
				processor.setUpdateQualifiedNames(
						fQualifiedNameCheckbox.getEnabled()
								&& fQualifiedNameCheckbox.getSelection());
			}
		}
		...
		private void addUpdateReferenceComponent(Composite result) {
			...
			fReferenceCheckbox
					.setText(ReorgMessages.ScriptMoveAction_update_references);
		...
		private void addUpdateQualifiedNameComponent(Composite parent,
				int marginWidth) {
			final ScriptMoveProcessor processor = getScriptMoveProcessor();
			if (!processor.canEnableQualifiedNameUpdating()
					|| !processor.canUpdateQualifiedNames())
				return;
			fQualifiedNameCheckbox = new Button(parent, SWT.CHECK);
			int indent = marginWidth + fQualifiedNameCheckbox
			...
			fQualifiedNameCheckbox
					.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
			fQualifiedNameCheckbox
					.setSelection(processor.getUpdateQualifiedNames());

			fQualifiedNameComponent = new QualifiedNameComponent(parent,
					SWT.NONE, processor, getRefactoringSettings());
			fQualifiedNameComponent
					.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
			GridData gd = (GridData) fQualifiedNameComponent.getLayoutData();
			gd.horizontalAlignment = GridData.FILL;
			gd.horizontalIndent = indent;
			updateQualifiedNameUpdating(processor,
					processor.getUpdateQualifiedNames());
		...
		private void updateQualifiedNameUpdating(
				final ScriptMoveProcessor processor, boolean enabled) {
		...
		public void createControl(Composite parent) {
			...
			boolean showDestinationTree = !getScriptMoveProcessor()
					.hasDestinationSet();
			if (showDestinationTree) {
				fCreateTargetQuery = getScriptMoveProcessor()
						.getCreateTargetQuery();
			...
			addUpdateQualifiedNameComponent(result,
					((GridLayout) result.getLayout()).marginWidth);
			setControl(result);
			Dialog.applyDialogFont(result);
		}
		...
		private void doNewButtonPressed() {
			Object newElement = fCreateTargetQuery
					.getCreatedTarget(fDestination);
			if (newElement != null) {
