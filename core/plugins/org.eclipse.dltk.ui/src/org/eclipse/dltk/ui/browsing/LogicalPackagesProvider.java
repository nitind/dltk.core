...
abstract class LogicalPackagesProvider
		implements IPropertyChangeListener, IElementChangedListener {
	...
	/**
	 * Adds the given fragments to the internal map. Existing fragments will be
	...
	protected String getKey(IScriptFolder fragment) {
		return fragment.getElementName()
				+ fragment.getScriptProject().getElementName();
	}

	/**
	 * Returns the logical package for the given package fragment or
	 * <code>null</code> if it is not grouped by logical package.
	 *
	 * @param fragment
	...
	/**
	 * Combines packages with same names into a logical package which will be
	 * added to the resulting array. If a package is not yet in this content
	 * provider then the package fragment is added to the resulting array.
	 *
	 * @param packageFragments
	 *                             the package fragments to combine
	 * @return an array with combined (logical) packages and package fragments
	 */
	protected Object[] combineSamePackagesIntoLogialPackages(
			IScriptFolder[] packageFragments) {
		...
		for (int i = 0; i < packageFragments.length; i++) {
			...
			if (lp != null) {
				...
				String key = getKey(fragment);
				IScriptFolder frag = (IScriptFolder) fMapToPackageFragments
						.get(key);
	...
	protected boolean isInCompoundState() {
		// XXX: for now we don't offer a preference might become a view menu
		// entry
	...
	abstract protected void processDelta(IModelElementDelta delta)
