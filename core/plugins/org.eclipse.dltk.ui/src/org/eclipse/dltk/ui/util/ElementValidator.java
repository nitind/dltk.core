...
public class ElementValidator {
	 ...
	 * @param parent
	 *            a parent shell used to present a dialog to the user if the
	 *            element is not in sync
	 * @param title
	 *            a dialog's title used to present a dialog to the user if the
	 *            element is not in sync
	 * @return boolean <code>true</code> if the element is in sync with the file
	 *         system. Otherwise <code>false</code> is returned
	 */
	public static boolean checkInSync(IAdaptable element, Shell parent,
			String title) {
	 ...
	 * @param parent
	 *            a parent shell used to present a dialog to the user if one of
	 *            the elements is not in sync
	 * @param title
	 *            a dialog's title used to present a dialog to the user if one
	 *            of the elements is not in sync
	 * @return boolean <code>true</code> if the all elements are in sync with
	 *         the file system. Otherwise <code>false</code> is returned
	 */
	public static boolean checkInSync(IAdaptable[] elements, Shell parent,
			String title) {
		return checkInSync(getResources(elements), parent, title);
	}

	/**
	 * Checks if the given element is read-only and if so the methods tries to
	 * make the element writable by calling validate edit. If
	 ...
	 *      java.lang.Object)
	 */
	public static boolean checkValidateEdit(IModelElement[] elements,
			Shell parent, String title) {
	 ...
	 */
	public static boolean check(IModelElement element, Shell parent,
			String title, boolean editor) {
		return check(new IModelElement[] { element }, parent, title, editor);
	}
	 ...
	 * @see #checkValidateEdit(IJavaElement[], Shell, String)
	 */
	public static boolean check(IModelElement[] elements, Shell parent,
			String title, boolean editor) {
	...
	private static boolean checkInSync(IResource[] resources, Shell parent,
			String title) {
		IStatus status = Resources.checkInSync(resources);
		if (status.isOK())
			return true;
		ErrorDialog.openError(parent, title,
				DLTKUIMessages.ElementValidator_cannotPerform, status);
		return false;
	}

	private static boolean checkValidateEdit(IResource[] resources,
			Shell parent, String title) {
